...
public class Outline {
	...
	public Outline(PbcsApplication application) {
		...
		for (PbcsDimension dimension : dimensions) {
			...
			logger.info("{} has {} children", rootMember.getName(), rootMember.getChildren().size());
			...
			while (!members.isEmpty()) {
				...
				logger.info("Processing {}, has {} children, level: {} in dim {}", current.getName(), current.getChildren().size(), current.getLevel(), current.getDimensionName());
